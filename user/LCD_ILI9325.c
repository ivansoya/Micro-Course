...
void delay(void) 
{
	volatile uint32_t count=400000;//50 msec
    while(count--) __nop();
}
...
void lcd_draw_picture(uint16_t pos_x, uint16_t pos_y, uint16_t size_x, uint16_t size_y, char* pixels) 
{
    uint16_t i = 0;
    for (uint16_t y = pos_y; y < size_y + pos_y; y++) {
        for (uint16_t x = pos_x; x < size_x + pos_x; x++) {
            if (pixels[i] == 0) {
                lcd_set_pixel(y, 320 - x, LCD_COLOR_WHITE);
            }
            else {
                lcd_set_pixel(y, 320 - x, LCD_COLOR_BLACK);
            }
            i++;
        }
    }
...
void lcd_draw_picture_by_coords(uint16_t pos_x, uint16_t pos_y, uint16_t size, char* pixels) 
{
    for (uint16_t i = 0; i < size; i += 2) {
        lcd_set_pixel(pos_y + pixels[i+1], 320 - (pos_x + pixels[i]), LCD_COLOR_BLACK);
    }
}

void lcd_draw_mirror_picture_by_coords(uint16_t pos_x, uint16_t pos_y, uint16_t size, char* pixels) 
{
    for (uint16_t i = 0; i < size; i += 2) {
        lcd_set_pixel(pos_y + pixels[i+1], (pos_x + pixels[i]), LCD_COLOR_BLACK);
    }
}
