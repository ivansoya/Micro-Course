...
void putbuf_char(uint16_t* bufchar, uint8_t ASCI,uint16_t bkcolor, uint16_t charcolor, _Bool isRotated)//16x8=128
{
	uint16_t i, j;
    uint8_t buffer[16], tmp_char;
    uint16_t bufcolor[16*8];
    ...
    if (isRotated == 1) {
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 8; j++) {
                bufcolor[(7-j)*16 + i] = bufchar[i*8+j];
            }
        }
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 8; j++) {
                bufchar[i*8+j] = bufcolor[i*8+j];
            }
        }
    }
}
...
void LCDprintstr(char* str, int16_t y,uint16_t bkcolor, uint16_t charcolor)//240/8=30 символов в строке
{
	...
	for(nchr=0 ; nchr<sizestr; nchr++){
		if( (str[nchr] >= 0x20 ) && (str[nchr] <=0x7E) ){
            ...
            //for(icol=0;icol<8;icol++){
			//	for(irow=0;irow<16;irow++){
			//		bstr[(nchr*8)+irow*(sizestr*16)+icol]=bufchar[irow*16+icol];
			//	}
			//}
		}
		else
	...
	//lcd_fill_region(y, 0, y + 8, 8*sizestr, bstr);
    lcd_fill_region(0, y, 8*sizestr, y + 16, bstr);
}

//печать перевернутого символа на экран
void LCDprintChar(char str, int16_t x, int16_t y, uint16_t bkcolor, uint16_t charcolor, 
    _Bool isRotated)//240/8=30 символов в строке
{
    uint16_t bstr[16*8];
    uint16_t bufchar[8*16];
    if ((str >= 0x20) && (str <= 0x7E)) {
        if (isRotated == 0) {
            putbuf_char(bufchar, str, bkcolor,charcolor, 0);
            for(int irow=0;irow<16;irow++){
                for(int icol=0;icol<8;icol++){
                    bstr[irow*8+icol]=bufchar[irow*8+icol];
                }
            }
            lcd_fill_region(0, y, 8, y + 16, bstr);    
        } 
        else {
            putbuf_char(bufchar, str, bkcolor,charcolor, 1);
            for(int icol=0;icol<8;icol++){
                for(int irow=0;irow<16;irow++){
                    bstr[irow+icol*16]=bufchar[irow+icol*16];
                }
            }
            //lcd_fill_region(y, 0, y + 16, 8, bstr);
            lcd_fill_region(x, y, x + 16, y + 8, bstr);
        }
    }
    else return;
}
